...
export class SwapFinder {
    ...
    private processPairedItemsInLayer(q: QueueItem) {
        for (let i = q.percentageIndex; i >= 0; --i) {
            ...
            if (percentage > q.remainingPercentage || !this.percentagesToSortedQuotes.has(percentage)) {
                continue;
            }
    ...
    private processPairedItem(q: QueueItem, percentage: number, index: number) {
        ...
        const newRemainingPercentage = q.remainingPercentage - percentage;
    ...
    private initQueueWithHighestQuotes() {
        for (let i = this.percentages.length - 1; i >= 0; --i) {
            this.insertHighestQuoteForPercentageIfExist(i);
            this.insertSecondHighestQuoteForPercentageIfExist(i);
        }
    }

    private insertHighestQuoteForPercentageIfExist(percentageIndex: number) {
    ...
    private insertSecondHighestQuoteForPercentageIfExist(percentageIndex: number) {
        const percentage = this.percentages[percentageIndex];
    ...
    private getSortedQuotes(routeWithQuotes: TRouteWithQuote[]) {
        const percentageToQuotes = new Map<number, TRouteWithQuote[]>();
        routeWithQuotes.forEach(routeWithQuote => {
            const percentage = routeWithQuote.amount.percentage;
            if (!percentageToQuotes.has(percentage)) {
                percentageToQuotes.set(percentage, []);
            }
            percentageToQuotes.get(percentage)?.push(routeWithQuote);
        });
        percentageToQuotes.forEach((value, _) => {
            value.sort((a, b) => {
